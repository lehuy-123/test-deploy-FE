...
const EditBlog = () => {
      ...
      try {
        ...
        if (blog.images && blog.images.length) {
          setPreviewMainImages(
            blog.images.map(img => img.startsWith('http') ? img : `https://test-deploy-be.render.com${img}`)
          );
        }
      } catch (error) {
    ...
    for (let file of files) {
      ...
      try {
        ...
        if (url) {
          uploaded.push({
            file,
            url: url.startsWith('http') ? url : `https://test-deploy-be.render.com${url}`,
          });
        }
      } catch {}
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`https://test-deploy-be.render.com/api/blogs/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
